...
public interface IRender
{
    ...
    void UpdateTexture(TextureHandle handle, int width, int height, ReadOnlySpan<byte> data);
    void DestroyTexture(TextureHandle handle);
    void BindTexture(int slot, TextureHandle handle);

    // === Texture Array Management ===
    TextureHandle CreateTextureArray(int width, int height, int layers);
    void UpdateTextureArrayLayer(TextureHandle handle, int layer, ReadOnlySpan<byte> data);
    void BindTextureArray(int slot, TextureHandle handle);

    // === Shader Management ===
    ShaderHandle CreateShader(string name, string vertexSource, string fragmentSource);
    ...
    void EndScenePass();
    void Composite(ShaderHandle compositeShader);
}
